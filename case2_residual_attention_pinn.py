...
OUT_DIR = "outputs_case9_residual_attention_pinn_mms_ns_2d"
...
class ResidualAttentionPINN(nn.Module):
    def __init__(self):
        ...
        self.stem = nn.Linear(TOKEN_EMBED_DIM, HEAD_HIDDEN)
        self.blocks = nn.ModuleList([ResidualAttentionBlock(HEAD_HIDDEN) for _ in range(4)])
...
def main() -> None:
    ...
    with open(os.path.join(LOG_DIR, "metrics.txt"), "w", encoding="utf-8") as f:
        f.write("MMS-based Residual-Attention PINN metrics\n")
    ...
    with open(os.path.join(LOG_DIR, "pde_and_exact_solution.txt"), "w", encoding="utf-8") as f:
        ...
        f.write("psi(x,y) = x^2(1-x)^2 y^2(1-y)^2 [ 0.38 sin(pi x) sin(pi y) + 0.16 tanh(beta1(x-0.30+0.05 sin(2 pi y))) - 0.14 tanh(0.78 beta1(x-0.69-0.06 cos(2 pi y))) + 0.10 exp(-((x-0.50)^2)/0.060) sin(pi y) ]\n")
        f.write("u(x,y) = d psi / d y\n")
        f.write("v(x,y) = - d psi / d x\n")
        f.write("p(x,y) = 0.50 cos(pi x) cos(0.7 pi y) + 0.16 tanh(beta2(x-0.52)) + 0.10 sin(pi x y) + 0.08 exp(-((x-0.56)^2+(y-0.44)^2)/0.080)\n")
        f.write("T(x,y) = 1.00 + 0.18 tanh(beta3(x-0.32+0.04 cos(2 pi y))) - 0.15 tanh(0.80 beta3(x-0.68-0.05 sin(2 pi y))) + 0.12 sin(pi x) sin(0.5 pi y) + 0.07 exp(-((x-0.54)^2/0.050 + (y-0.76)^2/0.030))\n")
        f.write("phi(x,y) = 0.30 tanh(beta4(x-0.25+0.06 sin(2 pi y))) - 0.24 tanh(0.72 beta4(x-0.72-0.04 cos(2 pi y))) + 0.10 cos(0.5 pi x) sin(pi y) + 0.06 exp(-((x-0.48)^2/0.040 + (y-0.28)^2/0.025))\n\n")
