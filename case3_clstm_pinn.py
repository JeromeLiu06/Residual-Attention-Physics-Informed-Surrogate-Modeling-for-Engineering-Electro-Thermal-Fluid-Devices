...
OUT_DIR = "outputs_case15_clstm_pinn_mms_ns_2d"
...
def exact_fields(xy: torch.Tensor) -> torch.Tensor:
    ...
    relay_center = (
        0.30
        + 0.11 * torch.tanh(6.0 * (x - 0.33))
        + 0.09 * torch.tanh(6.0 * (x - 0.67))
    )

    psi_bg = 0.40 * torch.sin(math.pi * x) * torch.sin(math.pi * y)
    psi_curve = 0.20 * torch.tanh(
        SHOCK_BETA_1 * (y - relay_center)
    )
    psi_left = 0.16 * torch.exp(
        -((x - 0.22) ** 2 / 0.016 + (y - 0.30) ** 2 / 0.026)
    )
    psi_mid = 0.14 * torch.exp(
        -((x - 0.52) ** 2 / 0.018 + (y - 0.50) ** 2 / 0.020)
    )
    psi_right = 0.12 * torch.exp(
        -((x - 0.80) ** 2 / 0.020 + (y - 0.68) ** 2 / 0.022)
    ) * torch.sin(1.5 * math.pi * (x + 0.6 * y))

    psi = envelope * (psi_bg + psi_curve + psi_left + psi_mid + psi_right)
    ...
    p = (
        0.48 * torch.cos(math.pi * x) * torch.cos(0.75 * math.pi * y)
        + 0.16 * torch.tanh(
            SHOCK_BETA_4 * (x - 0.76 + 0.05 * torch.sin(2.0 * math.pi * y))
        )
        + 0.13 * torch.exp(-((x - 0.81) ** 2 / 0.012 + (y - 0.66) ** 2 / 0.018))
    ...
    T = (
        1.00
        + 0.22 * torch.tanh(
            SHOCK_BETA_3 * (x - 0.51 + 0.07 * torch.cos(2.0 * math.pi * y))
        )
        + 0.20 * torch.exp(-((x - 0.50) ** 2 / 0.016 + (y - 0.56) ** 2 / 0.020))
        + 0.11 * torch.sin(math.pi * x) * torch.sin(0.6 * math.pi * y)
        + 0.06 * x * (1.0 - x)
    )
    ...
    phi = (
        0.30 * torch.tanh(
            SHOCK_BETA_2 * (x - 0.22 - 0.08 * torch.sin(2.0 * math.pi * y))
        )
        + 0.28 * torch.exp(-((x - 0.18) ** 2 / 0.008 + (y - 0.30) ** 2 / 0.016))
