...
OUT_DIR = "outputs_case9_clstm_pinn_mms_ns_2d"
...
def exact_fields(xy: torch.Tensor) -> torch.Tensor:
    ...
    psi_bg = 0.38 * torch.sin(math.pi * x) * torch.sin(math.pi * y)

    psi_layer_1 = 0.16 * torch.tanh(
        SHOCK_BETA_1 * (x - 0.30 + 0.05 * torch.sin(2.0 * math.pi * y))
    )

    psi_layer_2 = -0.14 * torch.tanh(
        0.78 * SHOCK_BETA_1 * (x - 0.69 - 0.06 * torch.cos(2.0 * math.pi * y))
    )

    psi_bridge = 0.10 * torch.exp(-((x - 0.50) ** 2) / 0.060) * torch.sin(math.pi * y)

    psi = envelope * (psi_bg + psi_layer_1 + psi_layer_2 + psi_bridge)
    ...
    p = (
        0.50 * torch.cos(math.pi * x) * torch.cos(0.7 * math.pi * y)
        + 0.16 * torch.tanh(SHOCK_BETA_2 * (x - 0.52))
        + 0.10 * torch.sin(math.pi * x * y)
        + 0.08 * torch.exp(-((x - 0.56) ** 2 + (y - 0.44) ** 2) / 0.080)
    )


    T_layer_1 = torch.tanh(
        SHOCK_BETA_3 * (x - 0.32 + 0.04 * torch.cos(2.0 * math.pi * y))
    )

    T_layer_2 = torch.tanh(
        0.80 * SHOCK_BETA_3 * (x - 0.68 - 0.05 * torch.sin(2.0 * math.pi * y))
    )
    ...
    T = (
        ...
        + 0.07 * torch.exp(-((x - 0.54) ** 2 / 0.050 + (y - 0.76) ** 2 / 0.030))
    )

    phi_layer_1 = torch.tanh(
        SHOCK_BETA_4 * (x - 0.25 + 0.06 * torch.sin(2.0 * math.pi * y))
    )

    phi_layer_2 = torch.tanh(
        0.72 * SHOCK_BETA_4 * (x - 0.72 - 0.04 * torch.cos(2.0 * math.pi * y))
    )

    phi = (
        0.30 * phi_layer_1
        - 0.24 * phi_layer_2
        + 0.10 * torch.cos(0.5 * math.pi * x) * torch.sin(math.pi * y)
        + 0.06 * torch.exp(-((x - 0.48) ** 2 / 0.040 + (y - 0.28) ** 2 / 0.025))
    )
...
class CLSTMPINN(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv1d(TOKEN_EMBED_DIM, CONV_CHANNELS, kernel_size=3, padding=1)
        self.lstm = nn.LSTM(
        ...
        self.res_proj = nn.Linear(CONV_CHANNELS, LSTM_HIDDEN)
    ...
    def forward(self, xy: torch.Tensor) -> torch.Tensor:
        ...
        conv_in = emb.transpose(1, 2).contiguous()
        conv_out = torch.tanh(self.conv(conv_in)).transpose(1, 2).contiguous()
        with torch.backends.cudnn.flags(enabled=False):
            lstm_out, _ = self.lstm(conv_out)
        fused = lstm_out + self.res_proj(conv_out)
        z = fused[:, -1, :]
        return self.head(z)
...
def main() -> None:
    ...
    model = CLSTMPINN().to(DEVICE).to(DTYPE)
    ...
    with open(os.path.join(LOG_DIR, "metrics.txt"), "w", encoding="utf-8") as f:
        f.write("MMS-based c-LSTM-PINN metrics\n")
    ...
    with open(os.path.join(LOG_DIR, "pde_and_exact_solution.txt"), "w", encoding="utf-8") as f:
        ...
        f.write("psi(x,y) = x^2(1-x)^2 y^2(1-y)^2 [ 0.42 sin(pi x) sin(pi y) + 0.20 tanh(beta1(y-0.35-0.20 sin(pi x))) + 0.16 exp(-((x-0.28)^2/0.020 + (y-0.30)^2/0.030)) + 0.12 exp(-((x-0.72)^2/0.022 + (y-0.66)^2/0.020)) sin(2 pi (x-y)) ]\n")
        f.write("u(x,y) = d psi / d y\n")
        f.write("v(x,y) = - d psi / d x\n")
        f.write("p(x,y) = 0.52 cos(pi x) cos(0.8 pi y) + 0.22 tanh(beta2(x+y-1.02+0.08 sin(2 pi x))) + 0.10 exp(-((x-0.52)^2+(y-0.48)^2)/0.014) + 0.06 sin(pi x y)\n")
        f.write("T(x,y) = 1.00 + 0.30 exp(-((x-0.78)^2/0.010 + (y-0.74)^2/0.016)) + 0.16 tanh(beta3(y-0.70+0.08 cos(2 pi x))) + 0.12 sin(pi x) sin(0.5 pi y) + 0.07 x(1-x)\n")
        f.write("phi(x,y) = 0.34 exp(-((x-0.22)^2/0.008 + (y-0.26)^2/0.012)) + 0.28 tanh(beta4(x-0.26-0.10 sin(2 pi y))) + 0.10 cos(0.5 pi x) sin(pi y) + 0.06 x y (1-y)\n\n")
